 ...
 * of a JRule java file in $OH_HOME/automation/jrule/rules. This can cause the RuleGenerator the be
 * executed multiple times in quick succession, which is why a lock mechanism is in place to prevent multiple loads.
 * The RuleGenerator will wait for a configured time before starting rule generation to allow the system to stabilize
 * after startup.
 * <p>
 * The RuleGenerator can be configured with environment variables:
 * - JRX_STARTUP_WAIT: Time in milliseconds to wait before starting rule generation (default: 5000)
 * - JRX_RULE_WAIT: Time in milliseconds to wait per generated rule (default: 50)
 * - JRX_LOADED_WAIT: Time in milliseconds to wait after generating rules before setting loaded state (default: 2000)
 */
public class JRuleXprRuleGenerator extends JRule {

    private static final String GENERATOR_LOCK = "jrx-loader-lock";

    private static final long GENERATOR_LOCK_MS;
    ...
    static {
        ...
        String lockMs = System.getProperty("JRX_GENERATOR_LOCK_MS");
        GENERATOR_LOCK_MS = lockMs == null ? 20000L : Long.parseLong(lockMs);
        if (lockGenerator(GENERATOR_LOCK_MS, false)) {
            generate();
        }

    }

    private static boolean lockGenerator(long ms, boolean plus) {
        boolean locked = false;
        String lock = System.getProperty(GENERATOR_LOCK);
        Long lockEpoch = lock == null ? null : Long.parseLong(lock);

        long now = ZonedDateTime.now().toInstant().toEpochMilli();
        if (lockEpoch == null || now > lockEpoch || plus) {
            Long from = plus && lockEpoch != null ? lockEpoch : now;
            System.setProperty(GENERATOR_LOCK, Long.valueOf(from + ms).toString());
            LOGGER.info("JRuleXprRuleGenerator lock acquired");
            locked = true;
        } else {
    ...
    private static void generate() {
        int startupWaitMs = Integer
                .parseInt(Optional.ofNullable(System.getenv("JRX_STARTUP_WAIT")).orElse("5000"));
        int waitPerRuleMs = Integer.parseInt(Optional.ofNullable(System.getenv("JRX_RULE_WAIT")).orElse("50"));
    ...
    private static void generateItemRules(int waitPerRuleMs) {
        int loadedWaitMs = Integer.parseInt(Optional.ofNullable(System.getenv("JRX_LOADED_WAIT")).orElse("2000"));
        LOGGER.info("JRuleXprRuleGenerator generating item rules with waitPerRuleMs=" + waitPerRuleMs + " and genWaitMs="
                + loadedWaitMs);
        int generated = 0;
        String rulePath = Optional.ofNullable(System.getenv("OPENHAB_CONF")).orElseThrow(
                () -> new IllegalStateException("OPENHAB_CONF environment variable is not set"))
                + "/automation/jrule/rules";

        Instant start = Instant.now(); 
        ItemRuleGenerator generator = new ItemRuleGenerator(rulePath);

        lockGenerator(GENERATOR_LOCK_MS, true);
        ...
        if (generated > 0) {
            ...
            LOGGER.info("JRuleXprRuleGenerator: Generated " + generated + " rules in " + ms + " ms");
        } else {
